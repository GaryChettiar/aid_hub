...
class _DashboardState extends State<Dashboard> {
  ...
  DateTimeRange? _selectedDateRange; // null means no filter
  int _currentGroupIndex = 0;
  List<List<Map<String, dynamic>>> _chunkedInwards(
  ...
  void _performSearch() async {
    final batchSnapshots =
        await FirebaseFirestore.instance.collection('groupedInwards').get();
  ...
  Future<void> _loadInwardBatch(int batchIndex) async {
    ...
    inwardList.sort((a, b) {
      final aNo = int.tryParse(
              RegExp(r'\d{4}$').stringMatch(a['inwardNo'] ?? '') ?? '0') ??
          0;
      final bNo = int.tryParse(
              RegExp(r'\d{4}$').stringMatch(b['inwardNo'] ?? '') ?? '0') ??
          0;
  ...
  bool _matchesInwardSearch(String? inwardNo) {
    ...
    return inwardNo
            ?.toLowerCase()
            .contains(_inwardNoController.text.toLowerCase()) ??
        false;
  }

  bool _matchesSenderSearch(
      String? senderName, String? reference, String? description) {
    if (_nameController.text.isEmpty) return true;
    final query = _nameController.text.toLowerCase();
    final senderMatches = senderName?.toLowerCase().contains(query) ?? false;
    final referenceMatches = reference?.toLowerCase().contains(query) ?? false;
    final descriptionMatches =
        description?.toLowerCase().contains(query) ?? false;
    return senderMatches || referenceMatches || descriptionMatches;
  }
  ...
  Future<void> _showDateRangePickerDialog(BuildContext context) async {
    DateTime startDate = _selectedDateRange?.start ?? DateTime(2020);
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            ...
                            label: Text(DateFormat('MMM dd, yyyy').format(startDate)),
                            onPressed: () async {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            icon: const Icon(Icons.calendar_today),
                            label: Text(DateFormat('MMM dd, yyyy').format(endDate)),
                            onPressed: () async {
                              final picked = await showDatePicker(
                                context: context,
                                initialDate: endDate,
  ...
  Widget build(BuildContext context) {
    ...
    final currentGroup =
        groupedInwards.isNotEmpty ? groupedInwards[_currentGroupIndex] : [];
    return Scaffold(
      endDrawer: Drawer(
      ...
      appBar: AppBar(
        actions: [
          Builder(
            // <-- This gives access to Scaffold context
            builder: (context) {
              return Padding(
        ...
        children: [
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: TextField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    hintText:
                                        "Search by Sender Name or Bill Reference",
                                  ),
                                  onSubmitted: (value) => _performSearch(),
                                ),
                              ),
                              ...
                              Container(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  onPressed: () => _showDateRangePickerDialog(context),
                                ),
                              ),
                            ],
                          )
          ...
          Expanded(
            child: _lastFilteredDocs.isEmpty
                ? Center(
                    child: Text((_inwardNoController.text.isEmpty &&
